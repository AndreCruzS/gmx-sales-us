...
| D64 | **`opportunity_stage_events`**: append-only, trigger-written stage-transition log; no client may insert/update/delete it (RLS grants SELECT only). Powers the "opportunities advanced" metric required by both source documents | Stage transitions are unrecoverable from `updated_at`, which moves on any edit — without the log the headline metric would be a guess. Append-only + trigger-written means it adds no state a human maintains, so the Phase 5 gate ("pure derivation, no separate manual reporting") holds in substance. Approximating "advanced" from activity counts was considered and rejected: a number that looks authoritative and is wrong is worse than no number | ✅ **Locked** 2026-07-27 (client confirmed). Note for tenant onboarding: the log permanently records which membership changed which opportunity's stage and when — raise it if a tenant has an audit-retention position |
...
- **Phase 5 delivered:** management dashboard and weekly commercial review as
  **derived SQL views** (spec §15/§16, PDF §4) — `dashboard_pipeline`
  ...
  and stores nothing. **D64** (locked) adds the one new table:
