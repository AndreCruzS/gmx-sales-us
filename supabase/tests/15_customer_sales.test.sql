-- Dealer sales · tests 15: dashboard_customer_sales
...
select is(
  (select unit from dashboard_customer_sales
...
select is(
  (select won_qty from dashboard_customer_sales
    where customer_id = 'd0000000-0000-0000-0000-000000000001'),
  61000::numeric,
...
select is(
  (select won_qty from dashboard_customer_sales
    where customer_id = 'd0000000-0000-0000-0000-000000000002'),
  11000::numeric,
...
select is(
  (select out_qty from dashboard_customer_sales
    where customer_id = 'd0000000-0000-0000-0000-000000000002'),
  20000::numeric,
...
select is(
  (select open_qty from dashboard_customer_sales
    where customer_id = 'd0000000-0000-0000-0000-000000000001'),
  24000::numeric,
  'dealer_id carries a contractor deal to the dealer it runs through'
);

-- 6. …and the contractor itself is not somebody we sell to, so it has no row.
select is(
  (select count(*) from dashboard_customer_sales
    where customer_id = 'd0000000-0000-0000-0000-000000000004'),
  0::bigint,
  'a contractor never appears as a customer'
);

-- 6b. A DISTRIBUTOR is a customer too. GMX sells to the house as well as to
--     the door, and counting only dealers made every distributor foot vanish
--     from a screen whose whole job is "what are we selling and to whom".
select is(
  (select won_qty from dashboard_customer_sales
    where customer_id = 'd0000000-0000-0000-0000-000000000005'),
  36000::numeric,
  'a distributor is a customer and its volume is counted'
);
...
set local role authenticated;
select is(
  (select coalesce(sum(won_qty), 0) from dashboard_customer_sales
    where customer_id = 'd0000000-0000-0000-0000-000000000001'),
  31000::numeric,
...
set local role authenticated;
select is(
  (select won_value from dashboard_customer_sales
    where customer_id = 'd0000000-0000-0000-0000-000000000001'),
  196000.00::numeric,
...
select is(
  (select count(*) from dashboard_customer_sales
    where customer_id = 'd2000000-0000-0000-0000-000000000001'),
  0::bigint,
